...
class TextRCNN:
    def __init__(self, sequence_length, num_classes, vocab_size, word_embedding_size, context_embedding_size,
        ...
        with tf.name_scope("precision"):
            TP = tf.count_nonzero(self.predictions * tf.argmax(self.input_y, axis=1))
            FP = tf.count_nonzero(self.predictions * (tf.argmax(self.input_y, axis=1) - 1))
            precision = TP / (TP + FP)
	...
	# TJ Recall
        with tf.name_scope("recall"):
            TP = tf.count_nonzero(self.predictions * tf.argmax(self.input_y, axis=1))
            FN = tf.count_nonzero((self.predictions - 1) * tf.argmax(self.input_y, axis=1))
            recall = TP / (TP + FN)
            self.recall = recall
