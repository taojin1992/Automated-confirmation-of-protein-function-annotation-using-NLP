...
import time
Entrez.email = 'taojincs@gmail.com'
with open('269-nodup-func.txt', 'r') as f:
    pmids = [line.strip() for line in f]
...
titles = [pubmed_article['MedlineCitation']['Article']['ArticleTitle']  if 'Abstract' in pubmed_article['MedlineCitation']['Article'].keys() else pubmed_article['MedlineCitation']['Article']['ArticleTitle']  for pubmed_article in records['PubmedArticle']]
title_dict = dict(zip(pmids, titles))
abstract_dict = dict(zip(pmids, abstracts))
#print abstract_dict
with open('269-pub.txt','w') as log:
    for value in abstract_dict.values():
        #print(value)
        log.write('{}\n'.format(value.encode('utf-8')))  # for special char

with open('269-title.txt','w') as log:
...
import time
Entrez.email = 'taojincs@gmail.com'
with open('303-nodup.txt', 'r') as f:
    pmids = [line.strip() for line in f]
...
abstract_dict = dict(zip(pmids, abstracts))
#print abstract_dict
with open('303-pub.txt','w') as log:
    for value in abstract_dict.values():
        #print(value)
        log.write('{}\n'.format(value.encode('utf-8')))  # for special char

with open('303-title.txt','w') as log:
    for value in title_dict.values():
        #print(value)
        log.write('{}\n'.format(value.encode('utf-8')))  # for special char
